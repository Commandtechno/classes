...
export default function SectionPicker({ course, onSelect, onClose }: Props) {
  ...
  useState(() => {
    searchCourses({
      ...
      .then((data) => {
        const sameCode = data.results.filter(
          (r) => r.code === course.code && r.crn !== course.crn
        );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div
        className="relative bg-white rounded-xl shadow-2xl max-w-sm w-full max-h-[70vh] overflow-hidden"
      ...
      >
        ...
        <div className="overflow-y-auto max-h-[50vh] p-3 space-y-1.5">
          ...
          {!loading && sections.length === 0 && (
            <p className="text-sm text-gray-400 text-center py-8">No other sections found</p>
          )}
          {sections.map((s) => (
            <button
              key={s.crn}
